...
var pollIntervalMin = 6000000;  // 5 minutes - temporarily no background refresh needed.
var pollIntervalMax = 600000000;  // 1 hour
  ...
  try {
    xhr.onreadystatechange = function() {
	 ...
	 // alert("hello" + xhr.responseText);
		...
		var mailTitlePath = "/gmail:feed/gmail:entry/gmail:title";
		
		var gmailNodes=xmlDoc.evaluate(mailTitlePath, xmlDoc, gmailNSResolver, XPathResult.ANY_TYPE, null);
		var result=gmailNodes.iterateNext();

		var finalVal = "";
		while (result)
		  {
		  finalVal = finalVal + result.childNodes[0].nodeValue + "<br>";		  
		  result=gmailNodes.iterateNext();
		  }
		  
		//  alert("finalVal is" + finalVal);
		// var firstMail = xmlDoc.evaluate("/gmail:feed/gmail:entry/gmail:title",
         //   xmlDoc, gmailNSResolver, XPathResult.ANY_TYPE, null);
